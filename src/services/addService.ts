...
export const getAllAds = async (searchQuery: string, employementType: string): Promise<IAds> => {
  const url = `https://jobsearch.api.jobtechdev.se/search?q=${searchQuery}&${employementType}&offset=0&limit=10&stats.limit=20`;
...
//Gammal länk:
//`https://jobsearch.api.jobtechdev.se/search?q=${searchQuery}&offset=0&limit=20`
